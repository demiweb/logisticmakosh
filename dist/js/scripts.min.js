...
var header = document.querySelector('.inside-left');
...
$('.select-status select').niceSelect();

let sortUlLi = [...document.querySelectorAll('.select-status ul li')];
let colorsData = [...document.querySelectorAll('.data-colors p')];

function sortSelectClick() {
    if (!sortUlLi.length) {

    } else {
        sortUlLi.forEach((btn, k) => {
            btn.style.setProperty('--color', `${colorsData[k].innerHTML}`);
            if (btn.classList.contains('selected')) {
                btn.closest('.select-status').querySelector('.nice-select > span').style.setProperty('--color', `${colorsData[k].innerHTML}`);
            }
            btn.addEventListener('click', () => {
                ...
                // suggestOpt.click();
                btn.closest('.select-status').querySelector('.nice-select > span').style.setProperty('--color', `${colorsData[k].innerHTML}`);
                // console.log(btn.closest('.select-status').querySelector('select'));
...
let btnPrint = [...document.querySelectorAll('.btn-print')];
...
let mobHistory = [...document.querySelectorAll('.about-task__history')];

function historyControl() {
    if (mobHistory.length) {
        mobHistory.forEach((btn) => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
